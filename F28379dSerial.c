...
// KZ Excercise 8 and Final Project
uint16_t COMC_state = 0;
uint16_t received_COMC_count = 0;
uint16_t RX_data_in[4];
uint32_t wii_button_state;
uint16_t NewCOMCdata;
...
// KZ the keyboard input detection through UART
...
__interrupt void RXCINT_recv_ready(void)
{
    ...
    if (COMC_state == 0) {
        // KZ RXCdata is the first char of our 2 start bits
        if (RXCdata == 0xAA) {
            COMC_state = 1;
        }
        ...
        // KZ after first start bit, we check for the second
        if (RXCdata == 0x55) {
            COMC_state = 10; // into the read mode
            received_COMC_count = 0;
        }
        ...
        RX_data_in[received_COMC_count] = RXCdata;
        received_COMC_count++;
    }

    if (received_COMC_count >= 4) {
        received_COMC_count = 0;
        COMC_state = 0;
        wii_button_state = (((uint32_t)RX_data_in[3]) << 24) | (((uint32_t)RX_data_in[2]) << 16) | (((uint32_t)RX_data_in[1]) << 8) | ((uint32_t)RX_data_in[0]);
        NewCOMCdata = 1;  // Flag new data
    }
